function Test_Truck() {
    ...
    this.ground = new Hill({h:-16,friction: 0.3, restitution:0.5});
...
function Truck(pos){
    ...
    this.chassis = world.add({
        ...
        shapes:[
            ...
            {shape:'polygon', density:1, groupIndex:-1, vertices:[-0.19, 0.88, 3.51, 0.8, 4.75, 0.48, 4.83, -0.08, 4.77, -0.47, 4.45, -0.66, -0.2, -0.49]},//front
            {shape:'polygon', density:0.5, groupIndex:0, vertices:[-0.06, 1.91, 1.31, 1.86, 1.84, 1.42, 2.51, 0.83, 1.57, 0.85, -0.17, 0.88]},//top
            {shape:'polygon', density:0.5, groupIndex:0, vertices:[ -0.47, 1.9, -0.25, 1.89, -0.28, 0.89, -0.51, 0.9 ]},//b1
            {shape:'polygon', density:0.5, groupIndex:0, vertices:[ -0.76, 1.9, -0.53, 1.9, -0.57, 0.9, -0.79, 0.9 ]},//b2
            {shape:'polygon', density:0.5, groupIndex:0, vertices:[ -0.76, 1.89, -0.77, 1.7, -3.03, 0.96, -3.54, 0.96]},//b3
        ],
    });
    ...
    this.jointB = world.addJoint({
        ...
        enableMotor:true, maxMotorTorque:200,//180,
...
Truck.prototype = {
    ...
    update : function(input){
        this.jointB.SetMotorSpeed(4*Math.PI * (input.down ? 1 : input.up ? -1 : 0));
        this.jointF.SetMotorSpeed(4*Math.PI * (input.down ? 1 : input.up ? -1 : 0));
...
function Wheel(pos){
    ...
    for(var i=0; i<n; i++){
        ...
        this.part[i] = world.add({
            ...
            density:3.5, restitution:0.2, friction:0.95, 
            groupIndex : -1 
        });
